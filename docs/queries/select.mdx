...
<Banner type="warning">
  <strong>Important:</strong>
  When using `defaultPopulate` on collections with [Uploads](/docs/fields/upload) enabled, it is important to specify `filename: true` and `url: true`, otherwise Payload will not be able to fetch the image URL and populate the URL field in queries, instead returning `url: null`.
</Banner>
