...
<Banner type="warning">
  <strong>Important:</strong>
  When setting `defaultPopulate` on [Uploads](/docs/fields/upload) you must specify `filename: true` since Payload uses the filename to fetch the image from the API. Without this, Payload won't be able to populate the `url` of your upload and it will return `null` instead.
</Banner>
